import {
  javascript,
  html,
  css,
  reactjs,
  typescript,
  npcil,
  marquee,
  meshcraft,
  emfinity,
  tailwind,
  nodejs,
  cpp,
  clerk,
  solidity,
  boblob,
  jasder,
  jasderLight,
  smack,
  smackLight,
  threejs,
  python,
  sql,
  mypic,
  git,
  github,
  postmanapi,
  chart,
  oneplay,
  edelweisslife,
  
  aws,
  supabase,
  next,
  missing,
  scholarbridge,
  scholarbridgeDark,
  stakeligames,
  stakeligamesDark,
  docker,
  kafka,
  redis,
  orchardcore,
  squidex,
  threejsLight,
  githubLight,
  awsLight,
  kafkaLight,
  csharp,
  dotnet,
  streamlit
} from "../assets";

export const navLinks = [
  {
    id: "about",
    title: "About",
  },
  {
    id: "work",
    title: "Work",
  },
  {
    id: "opensource",
    title: "OSS",
  },
  {
    id: "heatmap",
    title: "Heatmap",
  },
  {
    id: "projects",
    title: "Projects",
  },
  {
    id: "technologies",
    title: "Tech",
  },
  {
    id: "contact",
    title: "Contact",
  },
];

const hero = [
  {
    title : "Soumil Mukhopadhyay",
    icon : mypic,
  },
];

const experiences = [
  {
     title: "Assoc. SDE",
     company_name: "Edelweiss LI",
     icon: edelweisslife,
     iconBg: "#E6DEDD",
     date: "July 2026 - Present",
     points: [
       "Onboarded as a full-stack .NET engineer, configured development environments and agile workflows.",
       "Building .NET portals like CMSs and Customer Portals using C#, MVC, CRUD, REST & RBAC.",
     ],

  },
  {
     title: "SWE Intern",
     company_name: "1 Play Global",
     icon: oneplay,
     iconBg: "#E6DEDD",
     date: "April 2025 - August 2025",
     points: [
       "Built entire website using Next.js, React, TypeScript, Three.js, TailWind & ShadCN across 7+ pages.",
       "Building for 4+ sub-sections/sub-pages and resources, covering 5+ verticals responsively.",
     ],

  },
  {
    title: "SWE Intern",
    company_name: "MeshCraft",
    icon: meshcraft,
    iconBg: "#383E56",
    date: "Dec 2024 - April 2025",
    points: [
      "Worked as a full-stack MERN developer intern, on the company portfolio and website, 3D animations.",
      "Among the top 12.5% applicants, worked on 10+ 3D assets and the marketplace & asset pages.",
    ],
  },
  {
    title: "IB Fellow",
    company_name: "Marquee Equity",
    icon: marquee,
    iconBg: "#E6DEDD",
    date: "Mar 2024 - April 2025",
    points: [
      "Completed 20+ corporate training sessions, learning financial modeling and deal structuring as well.",
      "Wrote 10+ research reports on concluded funding transactions in MLA Format; stipend-based pay.",
    ],
  },
  {
    title: "SWE Intern",
    company_name: "NPCIL",
    icon: npcil,
    iconBg: "#383E56",
    date: "May 2024 - Jul 2024",
    points: [
      "Developed a Process Monitor Module GUI with Python, on QtCreator IDE and SQL as database.",
      "Ran checks for 10+ system categories and processes with real-time system resources monitored."
    ],
  },
  {
    title: "Tech Team",
    company_name: "Emfinity",
    icon: emfinity,
    iconBg: "#E6DEDD",
    date: "Sep 2023 - Aug 2024",
    points: [
      "Conducted and attended daily tech sessions, designed 200+ Math questions and organized 5+ events.",
      "Engaged 450+ students, significantly boosting overall social media following growth by 20%.",
      
    ],
  },
  

];

const openSourceContributions = [
  {
    repo: "OrchardCMS/OrchardCore",
    repoApiUrl: "https://api.github.com/repos/OrchardCMS/OrchardCore",
    languagesApiUrl: "https://api.github.com/repos/OrchardCMS/OrchardCore/languages",
    pullApiUrl: "https://api.github.com/repos/OrchardCMS/OrchardCore/pulls/19348",
    pullRequestLink: "https://github.com/OrchardCMS/OrchardCore/pull/19348",
    title: "Fix admin root active menu state",
    description:
      "Resolved admin menu active state bugs by normalizing tenant URL prefixes and eliminating stale navigation hash fallbacks for SaaS environments.",
    image: orchardcore,
    fallback: {
      stars: 8148,
      additions: 92,
      deletions: 2,
      changedFiles: 3,
      language: "C#",
      status: "Merged",
      createdAt: "2026-06-07T06:13:46Z",
      mergedAt: "2026-06-09T20:28:50Z",
    },
  },
  {
    repo: "Squidex/squidex",
    repoApiUrl: "https://api.github.com/repos/Squidex/squidex",
    languagesApiUrl: "https://api.github.com/repos/Squidex/squidex/languages",
    pullApiUrl: "https://api.github.com/repos/Squidex/squidex/pulls/1323",
    pullRequestLink: "https://github.com/Squidex/squidex/pull/1323",
    title: "SQL OpenIddict login coverage",
    description:
      "Added shared EF OpenIddict regression coverage to verify token row saving without application reference, fixing the cannot login after setup issue.",
    image: squidex,
    fallback: {
      stars: 2509,
      additions: 51,
      deletions: 0,
      changedFiles: 1,
      language: "C#",
      status: "Merged",
      createdAt: "2026-06-07T06:13:46Z",
      mergedAt: "2026-06-09T20:28:50Z",
    },
  },
];


const projects = [
  {
    name: "SimulateRecruitAI",
    description:
      "Tech stack: Next, NeonDB, Docker, Clerk, Stripe, Tailwind, JSON, Gemini AI, API routes & Radix UI; achieved 90+ sign-ups.",
    tags: [
      {
        name: "next",
        color: "blue-text-gradient",
      },
      {
        name: "react",
        color: "text-[#228B22]",
      },
      {
        name: "radix-ui",
        color: "green-text-gradient",
      },
    ],
    image: jasder,
    imageDark: jasder,
    imageLight: jasderLight,
    imageClassDark: "object-contain bg-[#0d0c12]",
    imageClassLight: "object-contain bg-white",
    source_code_link: "https://github.com/Soumilgit/Soumilgit-AI-Interview-SAAS",
    demo_link: "https://soumilgit-ai-interview-saas.vercel.app/",
  },
  {
    name: "Customer Churn",
    description:
      "Analyzes 4,000 bank customer records via Python, Supabase, HTML/CSS & GPT OSS 120B, averaging 82.73% accuracy.",
    tags: [
      {
        name: "xgb",
        color: "green-text-gradient", 
      },
      {
        name: "smote",
        color: "text-[#228B22]",
      },
      {
        name: "full-stack",
        color: "blue-text-gradient",
      },
    ],
    image: smack,
    imageDark: smack,
    imageLight: smackLight,
    imageClassDark: "object-contain bg-[#101319]",
    imageClassLight: "object-contain bg-white",
    source_code_link: "https://github.com/Soumilgit/XYZ-Bank-Customer-Churn-Predictor",
    demo_link: "https://cust-churn-pred-bank.streamlit.app/",
  },
  {
    name: "StakeLiGames",
    description:
      "On-chain staking protocol for LinkedIn Games on the Sepolia testnet, utilizing Next.js, TailWind, Python and Solidity.",
    tags: [
      {
        name: "typescript",
        color: "blue-text-gradient",
      },
      {
        name: "python",
        color: "green-text-gradient",
      },
      {
        name: "solidity",
        color: "pink-text-gradient",
      },
    ],
    image: stakeligames,
    imageDark: stakeligamesDark,
    imageLight: stakeligames,
    imageClassDark: "object-contain bg-[#101319]",
    imageClassLight: "object-contain bg-white",
    source_code_link: "https://github.com/Soumilgit/StakeLiGames",
    demo_link: "https://stake-li-games.vercel.app/",
  },
  {
    name: "ScholarBridge",
    description:
      "Full-stack scholarship platform utilizing the PERN stack, TailWind, Cloudflare R2, JWT + Bcrypt; scalable & auditable.",
    tags: [
      {
        name: "postgres",
        color: "blue-text-gradient",
      },
      {
        name: "reactjs",
        color: "green-text-gradient",
      },
      {
        name: "nodejs",
        color: "pink-text-gradient",
      },
    ],
    image: scholarbridge,
    imageDark: scholarbridgeDark,
    imageLight: scholarbridge,
    imageClassDark: "object-contain bg-[#101319]",
    imageClassLight: "object-contain bg-white",
    source_code_link: "https://github.com/Team-C49/ScholarBridge-backup",
    demo_link: "https://tinyurl.com/2m7ae4n4",
  },
  {
    name: "Dhyan-Adhyaksh",
    description:
      "Developed using React Native and Expo, useful for tracking one's meditation habits and keep a record with affirmations.",
    tags: [
      {
        name: "react",
        color: "blue-text-gradient",
      },
      {
        name: "javascript",
        color: "green-text-gradient",
      },
      {
        name: "typescript",
        color: "pink-text-gradient",
      },
    ],
    imageClass: "object-contain bg-[#e0e0e0]",
    image: boblob,
    source_code_link: "https://github.com/Soumilgit/Meditation-App_React-Native",
    demo_link: "https://youtu.be/3r5finsx_Iw",
  },
  {
    name: "Missing Finder",
    description:
      "Deployed AWS Lambda/S3 stack with Python & Rekognition for live person matching, ~1s response at 99.9% confidence.",
    tags: [
      {
        name: "aws",
        color: "blue-text-gradient",
      },
      {
        name: "lambda",
        color: "green-text-gradient",
      },
      {
        name: "python",
        color: "text-[#228B22]",
      },
    ],
    imageClass: "object-contain bg-[#3156ad]",
    image: missing,
    source_code_link: "https://github.com/Soumilgit/Real-Time-Missing-Persons-Detection",
    demo_link: "https://youtu.be/QA1EuvTgKeA",
  }
];


const technologies = [
  {
    name: "HTML 5",
    icon: html,
  },
  {
    name: "CSS 3",
    icon: css,
  },
  {
    name: "JavaScript",
    icon: javascript,
  },
  {
    name: "React",
    icon: reactjs,
  },
  {
    name: "TailWind",
    icon: tailwind,
  },
  { name: "Docker", 
    icon: docker, 
  },
  {
    name: "Supabase",
    icon: supabase,
  },

  {
    name: "Node",
    icon: nodejs,
  },
  {
    name: "ThreeJS",
    icon: threejs,
    iconLight: threejsLight,
  },
  {
    name: "Python",
    icon: python,
  },
  {
    name: "C++",
    icon: cpp,
  },
  {
    name: "MySQL",
    icon: sql,
  },
  {
   name: "TypeScript",
   icon: typescript,
  },
  {
   name: "AWS",
   icon: aws,
   iconLight: awsLight,
  },
  {
    name: "Git",
    icon: git,
  },
  {
    name: "GitHub",
    icon: github,
    iconLight: githubLight,
  },
  {
    name: "Kafka",
    icon: kafka,
    iconLight: kafkaLight,
  },
  {
    name: "Redis",
    icon: redis,
  },
  {
    name: "Clerk",
    icon: clerk,
  },
  {
    name: "Solidity",
    icon: solidity,
  },
  {
    name: "Postman",
    icon: postmanapi,
  },
  {
    name: "ChartJS",
    icon: chart,
  },
  {
    name: "Next",
    icon: next,
  },
  {
    name: "C#",
    icon: csharp,
  },
  {
    name: ".NET",
    icon: dotnet,
  },
  {
    name: "Streamlit",
    icon: streamlit,
  },
];
export { hero, technologies, experiences, openSourceContributions, projects };
