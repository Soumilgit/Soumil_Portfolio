...
const Hero = () => {
  ...
  return (
    <section className="flex flex-col md:flex-row h-auto justify-start items-center px-2 sm:px-2 bg-cover bg-center mt-20">
      
      {/* Conditional Rendering Based on Device Type */}
      {isMobile ? (
        // Mobile Layout: ID Card below text and button
        <>
          {/* Right Side (Text, Button, Arrow) */}
          <div className="flex flex-col items-center md:items-start gap-4 md:ml-[18px] md:self-center mt-8 order-1">
            ...
            <p className={`${styles.heroSubText} mt-4 text-white-100 text-left`}>
              <ScrambledText text="I like tech and exploring." className={styles.scrambledText} />
            </p>
          ...
          {/* Left Side (ID Card) */}
          <div className="flex justify-left items-center mt-12 md:mt-0 order-2">
          ...
          {/* Add margin to increase gap */}
          <div className="mt-0"></div> {/* Increased gap here */}

        </>
        
      ) : (
        // Desktop and Tablet Layout: ID Card on the left, text and button on the right
        <>
          {/* Left Side (ID Card) */}
          <div className="flex justify-left items-center mt-1 md:mt-0 order-1 md:w-1/2">
            <div className="relative w-72 h-[40em] sm:w-96 sm:h-[38rem] cursor-pointer perspective-1000" onClick={handleCardClick}>
              <motion.div className="relative w-full h-full" animate={{ rotateY: flipped ? 180 : 0 }} transition={{ duration: 0.6 }} style={{ transformStyle: "preserve-3d" }}>
